...
from .models import SalerDetail, Product, ProductSize, SellerSlider, MyCart, WholeSaleProduct, category, Orders, WholeSaleProductOrders,DoctorRegistration,Petctscandetail,Ambulance
from django.contrib import messages
from django.contrib.auth.models import User
from .forms import SalerRegisterForm, SalerAddressForm, UpdateSalerDetailForm, UpdateSalerAccountDetailForm,DoctorRegisterForm,PetctscanForm,AmbulanceBookingForm
	...
	if request.user.is_authenticated:
		...
		if request.method == 'POST':
			...
			if form.is_valid():
				...
				usr.save()
				if username.isdigit():
					SalerDetail(user=usr,mobile=username,gst_Number=gst,shop_Name=shop).save()
				else:
					usr.email = username
					usr.save()
	...
	if request.user.is_authenticated:
		...
		if request.method == 'POST':
			...
			if form.is_valid(): 
				...
				usr.save()
				DoctorRegistration(user=usr,mobile=username,clinic_Address=clinic_Address,locality=locality,pincode=pincode,city=city,state=state,letter_head=letterhead).save()
				messages.success(request, f'Account is Created for {username}')
				return redirect('login')
		else:
			form = DoctorRegisterForm()
	return render(request, 'saler/doctor_signup.html', {'form':form, 'title':'Register As Doctor'})
...
def petctscan_booking(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid(): 
			...
			Petctscandetail(mobile=username,residential_address=residential_address,pincode=pincode,city=city,state=state,patient_name=patient_name,relative_name=relative_name,relative_number=relative_number,relation=relation,services=services,appointment_slot=appointment_slot,appointment_date=appointment_date).save()
			#messages.success(request, f'Data Submitted')
			return redirect('/')
	else:
		form = PetctscanForm()
	return render(request, 'saler/petctscan_booking.html', {'form':form, 'title':'Book Appointment '})
...
def ambulance_booking(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid(): 
			...
			Petctscandetail(mobile=username,residential_address=residential_address,pincode=pincode,city=city,state=state).save()
			#messages.success(request, f'Data Submitted')
			return redirect('/')
	else:
		form = AmbulanceBookingForm()
	return render(request, 'saler/ambulance_booking.html', {'form':form, 'title':'Book Appointment '})
